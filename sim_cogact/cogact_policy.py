...
class CogACTInference:
    def __init__(
        ...
        cot_version: str = "v2",
        cot_generate_sample: bool = False,
        cot_allow_prefix: bool = False,
        **kwargs,
    ) -> None:
        ...
        self.vla = load_vla(
            # choose from ['CogACT/CogACT-Small', 'CogACT/CogACT-Base', 'CogACT/CogACT-Large'] or the local path
            saved_model_path,
            load_for_training=False,
            # choose from ['DiT-Small', 'DiT-Base', 'DiT-Large'] to match the model weight
        ...
        self.chunked_actions = deque(maxlen=self.action_chunk_window)
        ...
        self.cot_generate_sample = cot_generate_sample
        self.cot_allow_prefix = cot_allow_prefix
        print(
            f"#########################. inference params. ################### \
            \nself.action_ensemble:{self.action_ensemble}, self.action_chunk: {self.action_chunk}  \
            \nself.cot_version:{self.cot_version}, self.cot_generate_sample: {self.cot_generate_sample}, self.cot_allow_prefix: {self.cot_allow_prefix} \
            \n#########################. inference params. ###################"
        )
        ...
        if self.cot_version is not None:
            ...
            if self.cot_generate_sample > 0:
                if (timestep + 1) % self.cot_generate_sample == 0:
                    do_sample = True
            allow_prefix = False
            if self.cot_allow_prefix > 0:
                if (timestep) % self.cot_allow_prefix == 0:
                    allow_prefix = False
                else:
                    allow_prefix = True
            raw_actions, normalized_actions = self.vla.predict_action_cot(
                image=image,
