...
def make_dataset_from_rlds(
    ...
    *,
    ...
    if use_cot:
        assert os.path.exists(cot_file_path), f"cot_file_path {cot_file_path} does not exist"
        with open(cot_file_path, "r") as f:
            cot_dataset = json.load(f)

        def make_tf_dict(raw_dict):
            print("Building the cot dict...")
            keys = []
            values = []

            def reasoning_dict_to_str(d):
                tags = get_cot_tags_list(keyframe=True)[:-1]  # exclude ACTION
                database_keys = get_cot_database_keys(keyframe=True)
            ...
            for file_name in raw_dict.keys():
                for episode_id in raw_dict[file_name].keys():
                    ...
                    for i in raw_dict[file_name][episode_id]["reasoning"].keys():
                        ...
                        if "subtask_keyframe" in trajectory_features.keys():
                            if trajectory_features["subtask_keyframe"] is not None:
                                if 0 <= int(i) < len(trajectory_features["subtask_keyframe"]):
                                    reasoning_dict["subtask_keyframe"] = trajectory_features["subtask_keyframe"][int(i)]
                                else:
                                    reasoning_dict["subtask_keyframe"] = ""
                        if "move_keyframe" in trajectory_features.keys():
                            if trajectory_features["move_keyframe"] is not None:
                                if 0 <= int(i) < len(trajectory_features["move_keyframe"]):
                                    reasoning_dict["move_keyframe"] = trajectory_features["move_keyframe"][int(i)]
                                else:
                                    reasoning_dict["move_keyframe"] = ""
                        if "vision_keyframe" in trajectory_features.keys():
                            if trajectory_features["vision_keyframe"] is not None:
                                if 0 <= int(i) < len(trajectory_features["vision_keyframe"]):
                                    reasoning_dict["vision_keyframe"] = trajectory_features["vision_keyframe"][int(i)]
                                else:
                                    reasoning_dict["vision_keyframe"] = ""
        ...
        cot_dataset = make_tf_dict(cot_dataset)
    else:
        cot_dataset = None
    ...
    def restructure(traj):
        ...
        new_obs["traj_length"] = tf.repeat(traj_len, traj_len)
        ...
        if cot_dataset is not None:
            ...
            cot = cot_dataset.lookup(file_names + "_" + episode_ids + "_" + indices)
...
def make_interleaved_episodic_dataset(
    ...
    *,
    ...
    use_cot: bool = False,
    cot_file_path: Optional[str] = None,
    traj_group_size: Optional[int] = 0,
    use_optim_group_sample: Optional[bool] = False,
) -> dl.DLataset:
    """
    Creates an interleaved dataset from list of dataset configs (kwargs). Returns a dataset of batched frames.
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        if use_optim_group_sample:
            def group_sample(traj):
                ...
                # 少于 k 时 pad 最后一帧
                def pad_case():
                    idx = tf.concat([tf.range(T), tf.fill([traj_group_size - T], T - 1)], axis=0)
                    return idx

                # >= k 时随机抽取并排序
                def sample_case():
                    shuffled = tf.random.shuffle(tf.range(T))
                    return tf.sort(shuffled[:traj_group_size])

                indices = tf.cond(T < traj_group_size, pad_case, sample_case)
                return tf.nest.map_structure(
                    lambda tensor: tf.gather(tensor, indices, axis=0),
                    traj
                )

            dataset = dataset.map(
                lambda traj: group_sample(traj),
                num_parallel_calls=threads,
            )
