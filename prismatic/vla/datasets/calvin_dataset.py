...
class BaseCalvinDataset(Dataset):
    ...
    def __init__(
        ...
        with open('scripts/enrich_lang_annotations.json', 'r') as f:
    ...
    def __getitem__(self, idx: Union[int, Tuple[int, int]], fixed_seed=False) -> Dict:
        ...
        return dict(pixel_values=pixel_values, 
                    ...
                    raw_action=tgt_action,
                    lang=instruction,
                    proprio=proprio,
                    # hist_action=his_actions,
...
class IterableDiskCalvinDataset(IterableDataset):
    """
    Iterable 版本的 DiskCalvinDataset，内部复用原有的索引逻辑并支持多 worker 并行切分。
    并且所有底层属性（比如 dataset_statistics）都会被转发。
    用法与 DiskCalvinDataset 一致，只需在 DataLoader 中将 Dataset 换成本类即可。
    """

    def __init__(self, *args, **kwargs):
        super().__init__()
        # 复用原来的磁盘加载 Dataset
        self.base = DiskCalvinDataset(*args, **kwargs)
        # 预先生成所有可用的 “idx”
    ...
    def __iter__(self):
        ...
        # 3. 按打乱并切片后的顺序 yield 样本
        for idx in selected:
            yield self.base[idx]

    def __getattr__(self, name):
        """
        除了本类定义的方法以外，其它属性都从 self.base 中去取，
        这样就可以访问 dataset_statistics、naming_pattern 等。
        """
        # 避免无限递归
        if name in ("base", "indices"):
