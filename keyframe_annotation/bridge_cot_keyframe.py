...
if __name__ == "__main__":
        ...
        for demo_id in reasoning_data[filename].keys():
            ...
            if use_gaussian_smooth:
                if len(subtask_keyframe) != 0:
                    subtask_keyframe = gaussian_smooth(subtask_keyframe, window_size=3, sigma=1.0, threshold=0.1)
                if len(move_keyframe) != 0:
                    move_keyframe = gaussian_smooth(move_keyframe, window_size=3, sigma=1.0, threshold=0.1)
    ...
    with open("/data/lixiang10/embodiedCoT/embodied_features_bridge_keyframe_gauss.json", "w") as f:
        json.dump(reasoning_data, f, indent=4)
    print("Keyframe annotation completed and saved to /data/lixiang10/embodiedCoT/embodied_features_bridge_keyframe_gauss.json")
