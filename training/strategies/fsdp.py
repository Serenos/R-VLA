...
class FSDPStrategy(TrainingStrategy):
    ...
    def save_checkpoint(
        ...
        with FSDP.state_dict_type(self.vlm, self.fsdp_state_dict_type, self.fsdp_save_policy, self.fsdp_save_optimizer_policy):
            ...
            #     def save_opt_and_cleanup(ckpt, path):
            #         torch.save(ckpt, path)
            #         for f in path.parent.glob("*.optimizer"):
            #             if f != path:
            #                 f.unlink()
            #     self._save_executor.submit(save_opt_and_cleanup, optimizer_checkpoint, optimizer_path)
