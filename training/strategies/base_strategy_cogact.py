...
class TrainingStrategy(ABC):
    ...
    def run_vla_training(
        ...
        pin_memory: bool = False, # only calvin, True
        num_workers: int = 0, # only calvin, 16
    ) -> None:
        ...
        with tqdm(
            ...
            for train_idx, batch in enumerate(dataloader):
                ...
                with torch.autocast(
                    ...
                    if action_model:
                        head_loss, output = self.vlm(
                            ...
                            labels=batch["labels"],
                            proprio=batch["proprio"] if load_proprio else None,
                            output_hidden_states = True,
                            repeated_diffusion_steps=repeated_diffusion_steps,
                        )
                        vlm_loss = output.loss * vlm_loss_weight
                        loss = head_loss + vlm_loss
                    else:
                        assert not load_proprio
                        # [Contract] self.vlm.forward() must automatically compute `loss` and return!
                ...
                metrics.commit(loss=loss, head_loss=head_loss, vlm_loss=vlm_loss)
                ...
                if action_tokenizer is not None:
                    ...
                    if load_proprio:
                        extra_vlm_obs_num += 1
                    token_preds = output.logits[:, self.vlm.vision_backbone.num_patches + extra_vlm_obs_num : -1].argmax(dim=2)
