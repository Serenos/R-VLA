...
class VLAMetrics:
    def __init__(
        ...
        self.state = {
            "loss_raw": deque(maxlen=grad_accumulation_steps),
            "loss": deque(maxlen=window_size),
            "vlm_loss_raw": deque(maxlen=grad_accumulation_steps),
            "vlm_loss": deque(maxlen=window_size),
            "head_loss_raw": deque(maxlen=grad_accumulation_steps),
            "head_loss": deque(maxlen=window_size),
    ...
    def get_status(self,
                   ...
                   action_acc: Optional[torch.Tensor] = None,
                   cot_acc: Optional[torch.Tensor] = None,
                   ) -> str:
        # lr = self.state["lr"][-1] if len(self.state["lr"]) > 0 else 0
        # if loss is None:
        ...
        # return f"=>> [Epoch {self.epoch:03d}] Global Step {self.global_step:06d} - Loss :: {loss:.4f} - VLM Loss :: {vlm_loss:.4f} - Head Loss :: {head_loss:.4f} - LR :: {lr:.6f}"

        parts = [f"=>> [Epoch {self.epoch:03d}] Global Step {self.global_step:06d}"]
        if loss      is not None: parts.append(f"Loss: {loss:.4f}")
        if vlm_loss  is not None: parts.append(f"VLM Loss: {vlm_loss:.4f}")
        if head_loss is not None: parts.append(f"Head Loss: {head_loss:.4f}")
        if l1_loss   is not None: parts.append(f"Val L1 Loss: {l1_loss:.4f}")
        if action_acc is not None: parts.append(f"Action Acc: {action_acc:.4f}")
        if cot_acc   is not None: parts.append(f"COT Acc: {cot_acc:.4f}")
        # lr = self.state["lr"][-1] if len(self.state["lr"]) > 0 else 0
        ...
        **kwargs,
        ...
        for key, value in kwargs.items():
            if key == "loss":
                ...
                loss_val = value.detach()
                self.state["vlm_loss_raw"].append(loss_val)
                self.state["vlm_loss"].append(loss_val)
            elif key == "head_loss":
                loss_val = value.detach()
                self.state["head_loss_raw"].append(loss_val)
                self.state["head_loss"].append(loss_val)
            else:
    ...
    def push(self) -> str:
        ...
        loss_raw = torch.stack(list(self.state["loss_raw"])).mean().item()
        loss = torch.stack(list(self.state["loss"])).mean().item()
        vlm_loss_raw = torch.stack(list(self.state["vlm_loss_raw"])).mean().item()
        vlm_loss = torch.stack(list(self.state["vlm_loss"])).mean().item()
        head_loss_raw = torch.stack(list(self.state["head_loss_raw"])).mean().item()
        head_loss = torch.stack(list(self.state["head_loss"])).mean().item()
        l1_loss = torch.stack(list(self.state["l1_loss"])).mean().item() if self.state["l1_loss"] else 0.0
        action_accuracy = torch.stack(list(self.state["action_accuracy"])).mean().item() if self.state[
            "action_accuracy"] else 0.0
        cot_accuracy = torch.stack(list(self.state["cot_accuracy"])).mean().item() if self.state[
            "cot_accuracy"] else 0.0
        step_time, lr = np.mean(list(self.state["step_time"])), self.state["lr"][-1]
        status = self.get_status(
            ...
            head_loss=head_loss,
            l1_loss=l1_loss,
            action_acc=action_accuracy,
            cot_acc=cot_accuracy,
        )
        ...
        self.log(
            ...
            metrics={
                ...
                f"{prefix}/Loss": loss,
                f"{prefix}/VLM Loss": vlm_loss,
                f"{prefix}/Head Loss": head_loss,
                f"{prefix}/L1 Loss": l1_loss,
                f"{prefix}/Action Token Accuracy": action_accuracy,
                f"{prefix}/COT Accuracy": cot_accuracy,
                f"{prefix}/Loss (Raw)": loss_raw,
                f"{prefix}/VLM Loss (Raw)": vlm_loss_raw,
                f"{prefix}/Head Loss (Raw)": head_loss_raw,
                f"{prefix}/Learning Rate": lr,
                f"{prefix}/Step Time": step_time,
                **dataset_metrics,
            },
