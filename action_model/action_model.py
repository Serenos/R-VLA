...
class ActionModel(nn.Module):
    ...
    def loss(self, x, z):
        ...
        elif self.timestep_sample == "beta":
            alpha, beta, s = 1.5, 1.0, 0.999
            u = torch.distributions.Beta(alpha, beta).sample((x.size(0),)).to(x.device)
            timestep = (s * (1 - u) * (self.diffusion.num_timesteps - 1))
            timestep = timestep.clamp(min=0, max=self.diffusion.num_timesteps - 1).long()
